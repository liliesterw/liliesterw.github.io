...
// Render the chart
    chartObj.render = function () {
        ...
        updateTitle(0,0,0);
        function updateTitle(listing,tourist,year){
            if (listing != 0 ){
                d3.select(".chart-wrapper-listing").text(listing+"%");
                d3.select(".chart-wrapper-tourist").text(tourist+"%");        
                d3.select(".chart-wrapper-year").text((year-1)+ " to " + year);  
            }
            else{
                d3.select(".chart-wrapper-listing").text("113%");
                d3.select(".chart-wrapper-tourist").text("6%");        
                d3.select(".chart-wrapper-year").text("2017 to 2018"); 
            }
                          
        
        // d3.select(".chart-wrapper-info").text(listing);
        ...
        function mousemove() {
            ...
            try {
                ...
                // var dd = x0 - chartObj.xFunct(d0) > chartObj.xFunct(d1) - x0 ? d0 : d1;
            } catch (e) { return;}
            
            minY = chartObj.height;
            for (var y  in yObjs) {
                updateTitle(d.listing - chartObj.data[d.year-2016].listing,d.tourist - chartObj.data[d.year-2016].tourist, d.year)
                yObjs[y].tooltip
