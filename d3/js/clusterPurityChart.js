...
// https://gist.github.com/nswamy14/e28ec2c438e9e8bd302f
...
(function(){
	...
	d3.clusterpuritychart = function(){
		...
		var tooltipCircle = d3.select("body").append("div")
		...
		chart.renderPieChart = function(data){
			...
			arc.outerRadius(function (d) {
				if(fixAngleLayout){
					...
					return (radiusScale(d.data.total))*0.9 * (d.data.rate/100);
				}
			});
			...
			.on('mouseover',function(d){
					...
					.style("opacity", 1);
					tooltipCircle.html(
						"<p><strong> " + d.data.type+ " - " + d.parentData.year+"</strong></p>"+
						"<table><tbody><tr><td class='wide'>Total Listing :</td><td>"+
						 d.data.ele + " / " + d.data.total  + " </td></tr><tr><td class='wide'>Occupation Rate :</td><td>"
						 + parseInt(d.data.rate)  + "%</td></tr></tbody></table>")
					.style("left", (d3.event.pageX + 15) + "px")
					.style("top", (d3.event.pageY - 28) + "px")
				})
			.on('mouseout',function(d){
	...
	d3.legend = function () {
		...
		var chart = function(selection){
			...
			selection.each(function(data){
				...
				wrapEnter.append('text').attr('x',0).attr('y',0).text('Legend (clickable)')
